...
class GaussianMixture(torch.nn.Module):
    ...
    """
    def __init__(self, n_components, n_features, mu_init=None, var_init=None,pi_init=None, eps=1.e-6):
        """
    ...
    def _init_params(self):
        if self.mu_init is not None:
            ...
            self.mu = self.mu_init.to(DEVICE)
        else:
        ...
        if self.var_init is not None:
            ...
            self.var=self.var_init.to(DEVICE)
        else:
        ...
        if self.pi_init is not None:
            ...
            self.pi = self.pi_init.to(DEVICE)
        else:
    ...
    @staticmethod
    def load_params_from_np(weights_path):
        mu_weights = np.loadtxt(os.path.join(weights_path,"means.txt"))
        var_weights = np.loadtxt(os.path.join(weights_path,"cov.txt"))
        pi_weights = np.loadtxt(os.path.join(weights_path,"weights.txt"))

        mu_weights = torch.tensor(mu_weights).unsqueeze(0).float()
        var_weights = torch.tensor(var_weights).unsqueeze(0).float()
        pi_weights = torch.tensor(pi_weights).unsqueeze(0).unsqueeze(2).float()

        #print(mu_weights.size(),var_weights.size(),pi_weights.size())

        return mu_weights,var_weights,pi_weights
    ...
    def predict_utterance(self,x):
        x_flat = x.view(-1,x.size(2))
        x_flat = x_flat.unsqueeze(1).expand(x_flat.size(0), self.n_components, x_flat.size(1))
        p_k_flat = self.__p_k(x_flat, self.mu, self.var)
        p_k_flat = p_k_flat / (p_k_flat.sum(1, keepdim=True) + self.eps)
        p_k = p_k_flat.view(x.size(0),x.size(1),-1).sum(dim=1)
        _, predictions = torch.max(p_k, 1)
        
        return torch.squeeze(predictions)
    def sample(self,component,n_samples):
        """
        Sample from given components 
        component : k
        n_samples : int

        returns : tensor(d,n_samples)
        """

        dist = Normal(0,1)
        std = torch.sqrt(self.var)
        noise = dist.sample((self.n_features, n_samples))
        perturbation = std[component].unsqueeze(1) * noise + self.mu[component].unsqueeze(1)

        return perturbation
    
    def sample_batch(self,components_batch,n_samples):
        """
        Sample from given components 
        components_batch : k
        n_samples : int
        ...
        """
        dist = Normal(0,1)
        std = torch.sqrt(self.var)[0]
        mu = self.mu[0]
        components_batch = components_batch.to(std.device)
        noise = dist.sample((len(components_batch),self.n_features, n_samples)).to(std.device)
        std = std[components_batch].unsqueeze(2)
        mu = mu[components_batch].unsqueeze(2)
        perturbation = std * noise + mu
