...
class PNCC(object):
    def __init__(self, sr=16000, n_pncc=40, dct_type=2, norm='ortho', log_mels=False,
                 melkwargs=None):
        ...
        if self.n_pncc > self.MelSpectrogram.n_mels:
            raise ValueError('Cannot select more MFCC coefficients than # mel bins')
        #self.dct_mat = F.create_dct(self.n_pncc, self.MelSpectrogram.n_mels, self.norm)
        self.log_mels = log_mels
    ...
    def __call__(self, sig):
        ...
        Returns:
            spec_mel_db (Tensor): channels x hops x n_mels (c, l, m), where channels
                ...
                number of mel bins.
        """
        res = F.PNCC(sig, self.log_mels, self.s2db, self.n_pncc)
        return res
...
class Melspec(object):
    ...
    def __init__(self, srate, wlen=.0256, frate=100, nfft=512, nmel=40):
        ...
        self.hop = int(srate/frate)
        self.wind = hamming(int(wlen*srate), hop=self.hop)

    def _melspec(self,sig):
        """Compute melspec after cepstral-mean-norm."""
        pspec = stpowspec(sig, self.sr, self.wind, self.hop, self.nfft)
...
class LPBiquad(object):
    def __init__(self,sample_rate,cutoff_freq):
        self.sample_rate=sample_rate
        self.cutoff_freq=cutoff_freq
    def __call__(self,sig):
        return F.lowpass_biquad(sig,self.sample_rate,self.cutoff_freq)